...
void APP_Ultrasonic_UartRxDealforWork(void)
{
	...
	if(UART2.RxDataSucFlag==TRUE){
		...
		// {
			...
			printf("\r\n");
#endif
			if(UART2.RxBuff[1] == 0x81){
				s_tmp[4]=0x01;	//上报有车
			}else if(UART2.RxBuff[1] == 0x01){
				s_tmp[4]=0x00;	//上报无车
			}									//预留数据错误的提示上报，待完善
			...
			printf("\r\n");
#endif			
				APP_ULTRASONIC.RevBuffCashe=UART2.RxBuff[1];	//缓存接收数据的第二位

				if(APP_ULTRASONIC.GW_Down_Cmd_Flag==1){	//网关查询车位状态
					APP_ULTRASONIC.GW_Down_Cmd_Flag=0;	//清除网关查询命令标志位
					s_tmp[1]=ACK_PKG_CLASS;			//应答数据包类型
					s_tmp[2]=0x01;					//具体命令
					s_tmp[3]=0x80;					//状态
			...
			}else if (UART2.RxCnt>=6)	//查询地址类命令(暂未完善！)
			{
