...
//------固化的工作参数------
const static u8 SPIDER_WORK_DATA[37] = {0x00,0x00,0x00, 0xC8,0x00,0x00,0x00, 0x00,0x00,0x00,0x00,
			                  0x2C,0x01,0x3C,0x0A, 0x34,0x08,0x07,0x3C, 0x00,0xB4,0x00,0x3C, 0x00,0x2C,0x01,0x3C,
				          0x00,0x5A,0x00,0x00, 0x00,0x0A,0x00,0x1E, 0x00,0x00};
...
void APP_spider_UsartDealForInit(void)
{
	...
	case 13:
		...
		PRO_spider_BuildCMDForPar(CMD_TYPE_CONFIG,CONFIG_CMD_SETWORKPARAMETER,(u8 *)s_tmp,39);
		HAL_USART_SendStringN((u8 *)APP_SPIDER.TxUsartData,APP_SPIDER.TxUsartLeng,SPIDER_USART);// 工作参数查询
...
void APP_spider_uartDeal(void){
	...
	if(UART3.RxDataSucFlag==TRUE){
		...
		// 处理数据
		crc1= ALG_CmdCrcCal(&UART3.RxBuff[1],(UART3.RxLeng+2));
