...
static void DEVICE_init(void){
    ...
    HAL_TIM2_config();		//定时器2初始化----光电开关
	HAL_LED_config();		//LED初始化 PD7 PD6    低电平有效
    //HAL_RTC_Periph_Init();  //RTC初始化
    ...
    HAL_ADC_Init();         //检测内部参考电压
	HAL_eeprom_Init();
	//IWDG_config();			//使能独立看门狗
    APP_stalllock_GPIOInit();//车位锁GPIO初始化
...
static void WORK_VALUE_init(void){
	...
	Main_StrPar.Init_LED1_ShakeFlag=FALSE;
	  Main_StrPar.SpidOrH3v4Flag = TRUE;			//默认采用Spider
