...
*/
    ...
    if (((RTC->ISR2) & 0x4) == 0x4)//1s.
    {
      RTC->ISR1 = 0x0;//clear
      RTC->ISR2 = 0x0;
      
      if(HAL_adc_StrPar.CheckWorkVoltgeTimeCount != 0)
      {
	  HAL_adc_StrPar.CheckWorkVoltgeTimeCount--;
      }
      else if(HAL_adc_StrPar.CheckWorkVoltgeTimeCount == 0)
      {
	  HAL_adc_StrPar.CheckWorkVoltgeFlag = TRUE;
      }
    }
}
/**
...
/* In order to detect unexpected events during development,
   it is recommended to set a breakpoint on the following instruction.
*/
  HalTim2_StrPar.TIM_count++;//10us
  
  switch(HalTim2_StrPar.TIM_count)
  {
		case 0:
			break;
		case 1:
			...
			break;
		case 4:
			APP_StallLock_StrPar.K2_CacheValue = APP_StallLock_StrPar.K2_value;
			APP_StallLock_StrPar.K3_CacheValue = APP_StallLock_StrPar.K3_value;
		  	APP_StallLock_StrPar.K2_value = K2();
			APP_StallLock_StrPar.K3_value = K3();
			break;
		case 6:
			K1_OFF();
			break;
...
//-----------------200ms
  if(HalTim4_StrPar.TIM_count%200==0){
		...
		if(Main_StrPar.Init_LED_ShakeFlag == TRUE){//初始化模式LED闪烁
			LED1_XOR();	
			LED2_XOR();
		}
		if(Main_StrPar.Init_LED_AlternateShakeFlag == TRUE){
			HAL_LED_AlternateFlash();			//组网模式LED交替闪烁
		}
  }
...
//----------------1s	
  if(HalTim4_StrPar.TIM_count>=1000){
		...
		if(Main_StrPar.Init_LED1_ShakeFlag == TRUE){
			LED1_XOR();	//正常模式LED1秒闪提示
		}
  }
