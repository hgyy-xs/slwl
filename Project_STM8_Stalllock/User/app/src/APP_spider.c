...
void APP_spider_UsartDealForInit(void)
{
	...
	case 1:
		...
		APP_SPIDER.InitDealAddr++;				//下一步----确认终端为一级还是二级终端
		APP_SPIDER.UartRevOkFlag=FALSE;			//清除成功标志位
		APP_SPIDER.UartRevOverFlag=FALSE;		//清除超时标志位
		APP_SPIDER.UartRevOverCnt=500;			//设置超时时间
		APP_SPIDER.MF_Flag=FALSE;				//清除读取模块形态标志位
		break;
	case 2:
		if(APP_SPIDER.UartRevOkFlag==TRUE){
			APP_SPIDER.UartRevOkFlag=FALSE;
                        APP_SPIDER.UartRevOverCnt=0;
			if(APP_SPIDER.MF_Flag==TRUE){	//如果模块为一级终端
				APP_SPIDER.InitDealAddr++;	//下一步 
			}
			else{							//如果模块为二级终端
...
#endif				
			break;
  		case 0x02:					//一级单终端组网正常结束
#ifdef DEBUG_DDA_TO_MCU
			printf("\r\n【spider status】:only networking finish\r\n");
#endif				
			APP_SPIDER.Judge_ReadyOnNetFlag = TRUE;//成功组网，标志组网为真
                        		HAL_LED_AllOff();
			APP_SPIDER.Judge_Is_OnNetFlag = TRUE;	//已经组网
			break;
		case 0x03:					 //一级单终端组网超时1
#ifdef DEBUG_DDA_TO_MCU
			printf("\r\n【spider status】:only networking timeout 1\r\n");
#endif
			if(APP_SPIDER.Judge_Is_OnNetFlag == TRUE)//已经组网
			{
				APP_SPIDER.Judge_ReadyOnNetFlag = TRUE;//已有组网档案,本次组网超时1
                                HAL_LED_AllOff();
			}
			else
			{
				APP_SPIDER.Judge_ReadyOnNetFlag = TRUE;//没有组网档案,本次组网超时1,退出循环,debug
                                HAL_LED_AllOff();
				APP_SPIDER.Judge_SendOnNetCmdFlag = TRUE;
			}
			break;
		case 0x04:					 //一级单终端组网超时2
#ifdef DEBUG_DDA_TO_MCU
			printf("\r\n【spider status】:only networking timeout 2\r\n");
#endif
			if(APP_SPIDER.Judge_Is_OnNetFlag == TRUE)		//已经组网
			{
				APP_SPIDER.Judge_ReadyOnNetFlag = TRUE;//已有组网档案,本次组网超时2
                                HAL_LED_AllOff();
			}
			else
			{
				APP_SPIDER.Judge_ReadyOnNetFlag = TRUE;//没有组网档案,本次组网超时2,退出循环,debug
                                HAL_LED_AllOff();
				APP_SPIDER.Judge_SendOnNetCmdFlag = TRUE;
			}
			break;
	...
	if(data[6] &(1 << 5)){
		...
		case 0x40:			//下行一级M包
				 ...
				 PRO_spider_BuildCMDForNoPar(CMD_TYPE_QUERY,QUERY_CMD_GETDOWNMPACK);
                         ...
                         APP_SPIDER.DealSendCmdToSpiderAddr = 1;
				break;
			case 0x50: 		//下行一级P包通道开启
				#ifdef DEBUG_DDA_TO_MCU
					printf("\r\n【spider status】:down P packet open\r\n");
				#endif
				break;
			case 0x60: 		//下行一级P包通道关闭
				#ifdef DEBUG_DDA_TO_MCU
...
#ifdef DEBUG_MCU_STATUS                                               
                                                printf("\r\n");
                                                printf("DDA TO Ultrasonic U数据长度：%02x\n",APP_Screen.Tx_Data[7]);
#endif												
                                                HAL_RS485_TxNbyte( (u8 *)(APP_Screen.Tx_Data+8),(u16)APP_Screen.Tx_Data[7],USER_USART2);	//485串口下发命令到ultrasonic
...
#ifdef DEBUG_MCU_STATUS  
                                                printf("\r\n");
                                                printf("DDA TO Ultrasonic M数据长度：%02x\n",APP_Screen.Tx_Data[7]);
#endif
                                                HAL_RS485_TxNbyte( (u8 *)(APP_Screen.Tx_Data+8),(u16)APP_Screen.Tx_Data[7],USER_USART2);	//485串口下发命令到ultrasonic
                                            	// HAL_RS485_TxNbyte( (u8 *)(UART3.RxBuff+7),UART3.RxLeng-4,USER_USART2);	//485串口数据转发
                                            	APP_Screen_dealSendCmdToSpider();	//DDA终端回执  
