...
struct UART_STRUCT               UART1,UART2,UART3;
...
void HAL_UART1_IERecieve(u8 dat)
{
	if(UART1.RxDealStep>1){
			UART1.RxDataFlag = TRUE;
			UART1.RxTimeOutCnt=0;
		}
		switch(UART1.RxDealStep){
			case 0:break;
			case 1:  // 获取数据开头
				if(dat==0x59){
	
					UART1.RxCnt=0;
					UART1.RxBuff[UART1.RxCnt++]=dat;
					UART1.RxDealStep++;
					UART1.RxDataSucFlag=FALSE;
					}
				break;
			case 2:  // 数据长度高字节
				UART1.RxBuff[UART1.RxCnt++]=dat;
			...
			case 4:
				UART1.RxBuff[UART1.RxCnt++]=dat;
				if(UART1.RxCnt<(UART1.RxLeng+3+1+1)){
						
				}
				else if(UART1.RxCnt==(UART1.RxLeng+3+1+1)){
					UART1.RxDataSucFlag=TRUE;  // 完整数据标志
					UART1.RxDealStep=0; // 停止装载数据
					UART1.RxDataFlag = FALSE;
				}
				else{
					UART1.RxDealStep=1; 
				}
